...
rejected. Outputs are compressed image-folder archives with overviews. TaskVine
...
acknowledgement, cloud stop is requested before a capped guest-cleanup retry.
...
32 (up to one million rows each). The private specification is capped at 256 KiB.
Collection bounds each snapshot and decompressed archive to `output_bytes`
(default 256 MiB, maximum 1 GiB), 4096 archive members, and a 64 MiB validation
response. Guest service file-size limits are per file; configured worker disk is
