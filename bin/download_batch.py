...
class HostSignals:
    ...
    async def record(self, status_code: Optional[int], ttfb: Optional[float], bytes_downloaded: int = 0, is_conn_error: bool = False) -> None:
        async with self._lock:
            self._total += 1
...
class HostPolicyBBRController:
    ...
    async def step_interval(self, snap: dict[str, Any], now: float) -> None:
        ...
        if self.state == self.STARTUP:
            ...
            if had_overload:
                # Apply proportional backoff based on number of errors (up to 0.7^3)
                backoff_multiplier = min(n_overload, 1)
                ...
                err_detail = f"429x{snap['n429']}" if had_429 else f"overload x{n_overload}"
                print(f"[PolicyBBR] {self.host}: STARTUP→BACKOFF on {err_detail} (x{backoff_multiplier}) at rate={self.safe_rate:.1f}")
                return
            ...
            # Track consecutive latency degradations; only exit after 2 consecutive
            if latency_degraded:
                self._startup_consecutive_latency_degradations += 1
                if self._startup_consecutive_latency_degradations >= 10:
                    # Exit after 2 consecutive latency degradations → set ceiling, enter DRAIN
                    ...
                    print(f"[PolicyBBR] {self.host}: STARTUP latency warning ({self._startup_consecutive_latency_degradations}/2)")
                    return
            else:
        ...
        # ========== DRAIN PHASE ==========
            ...
            if had_overload:
                ...
                self.state = self.BACKOFF
                err_detail = f"429x{snap['n429']}" if had_429 else f"overload x{n_overload}"
                print(f"[PolicyBBR] {self.host}: DRAIN→BACKOFF on {err_detail} (x{backoff_multiplier})")
                return
        ...
        # ========== PROBE_BW PHASE (steady state) ==========
            ...
            if had_overload:
                ...
                self.state = self.BACKOFF
                err_detail = f"429x{snap['n429']}" if had_429 else f"overload x{n_overload}"
                print(f"[PolicyBBR] {self.host}: PROBE_BW→BACKOFF on {err_detail} (x{backoff_multiplier})")
                return
        ...
        # ========== PROBE_RTT PHASE ==========
            ...
            if had_overload:
                ...
                self.state = self.BACKOFF
                err_detail = f"429x{snap['n429']}" if had_429 else f"overload x{n_overload}"
                print(f"[PolicyBBR] {self.host}: PROBE_RTT→BACKOFF on {err_detail} (x{backoff_multiplier})")
                return
        ...
        # ========== BACKOFF PHASE ==========
            ...
            # With short intervals (0.5s), we rely on interval-based backoff.
            # If overload persists, each interval will back off again.
            if had_overload:
                ...
                self._backoff_until = now + self.backoff_cooldown_sec
                print(f"[PolicyBBR] {self.host}: BACKOFF {err_detail}, ceiling→{self.ceiling_rate:.1f}")
                return
...
async def download_one(
    *,
    ...
    try:
        ...
        # Per-host signal recording (TTFB + 429 ratio + connection errors)
        # Use the same _is_connection_error logic as _is_retryable for consistency
        is_conn_error = _is_connection_error(err)
        if host_ctrl is not None:
            await host_ctrl.signals.record(status_code=status, ttfb=trace_dict.get("ttfb"), bytes_downloaded=bytes_dl, is_conn_error=is_conn_error)
